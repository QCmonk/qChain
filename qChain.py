...
def atomic_sense(sig_amp=100, tau=1e-4, sig_freq=1e4, f_range=[250, 450, 1], parallel=True, savef=1000, verbose=True, **args):
    ...
    params = {"tstart":      0.0,               # time range to simulate over
              ...
              "rabi":       5000,               # dressing amplitude (Hz/2)
              ...
              "dete":       0.24,               # detuning truncation time
              "xlamp":         0,               # line noise amplitude
              "xlfreq":       50,                # line noise frequency              
              "xlphase":     0.0,                # start phase
              "zlamp":      1000,
              "zlfreq":       50,
              "zlphase":     0.1,
    ...
    for freq in freqs:
        ...
        #params["zlphase"] = np.round((-2*np.random.random() + 1)*np.pi/8, decimals=3)
